...
def _evaluate_sgdet_batch(
    ...
    """Convert RelTR outputs to evaluator format with post-processing.

    Matches the official RelTR post-processing (described in paper Section 3.3 and
    provided by the author as a draft replacement for engine.py:evaluate_rel_batch).

    Post-processing steps:
    1. Filter out "no-relation" triplets where argmax(rel_logits) is a bg token
    2. Softmax over predicate dims only (matching official protocol)
    3. Evaluator sorts by sub_score * obj_score * pred_score product
    """
    ...
    for i, target in enumerate(targets):
        ...
        # Official: softmax then exclude bg
        ...
        # Post-processing: filter "no-relation" triplets
        rel_dim = rel_logits.shape[-1]
        if rel_dim == rel_nums + 1:
            is_relation = (rel_logits.argmax(-1) != rel_nums)
        elif rel_dim == rel_nums + 2:
            bg0 = (rel_logits.argmax(-1) == 0)
            bg1 = (rel_logits.argmax(-1) == (rel_nums + 1))
            is_relation = ~(bg0 | bg1)
        else:
            is_relation = torch.ones(rel_logits.shape[0], dtype=torch.bool)

        rel_scores = _extract_relation_scores(rel_logits, rel_nums)
        pred_rel_labels = 1 + np.argmax(rel_scores, axis=1)

        # Apply no-relation filtering: zero out scores for "no-relation" triplets
        is_rel_np = is_relation.numpy()
        rel_scores_filtered = rel_scores.copy()
        rel_scores_filtered[~is_rel_np] = 0.0
        ...
        pred_entry = {
            ...
            "rel_scores": rel_scores_filtered.astype(np.float32),
        }
