...
This module ports the eval-only pieces of the official maskrcnn-benchmark
detector path that PE-NET needs before relation prediction:

  FPN features -> RPN proposals -> ROI box predictor -> relation proposals

It deliberately does not train the detector.  The modules are only used by
``PENet_Method`` when an official pretrained detector checkpoint is provided.
"""
...
class PENetSGDetProposalGenerator(nn.Module):
    ...
    def _filter_box_results(
        ...
        for cls_idx in range(1, self.num_classes):
            ...
            inds = inds[keep]
            result_boxes.append(boxes_j[keep])
            result_scores.append(scores_j[keep])
        ...
        if not result_boxes:
            ...
            return boxes_per_cls.new_zeros(0, 4), empty_long, empty_long
    ...
    def forward(
        ...
        if proposals_xyxy.numel() == 0:
            ...
            return PENetDetectorOutput(
                ...
                roi_feats=torch.zeros(0, box_extractor.out_channels, device=device),
            )
        ...
        selected_boxes = final_boxes_per_cls[torch.arange(labels.numel(), device=labels.device), labels]
        return PENetDetectorOutput(
            boxes=_xyxy_to_cxcywh_norm(selected_boxes, image_size),
            labels=labels.long(),
            obj_dists=class_logits[orig_inds],
            boxes_per_cls=final_boxes_per_cls,
            roi_feats=roi_feats[orig_inds],
        )
