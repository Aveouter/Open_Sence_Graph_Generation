...
1. Read `reproduction/how_to_use.md`.
2. Create or update a baseline issue from
   `reproduction/issues/baseline_reproduction_issue.md`.
3. Record decisions in `reproduction/adr/` when evidence is ambiguous or a
   method is deferred.
4. Fill the relevant templates in `reproduction/templates/`.
5. Verify checkpoint/config/evaluator compatibility before running metrics.
6. If alignment is missing, stop at audit/gap analysis. Do not manufacture a
