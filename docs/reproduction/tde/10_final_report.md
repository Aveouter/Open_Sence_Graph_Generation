...
**Superseded by stricter alignment audit:** this report documents source mapping
and random-init pipeline evidence only. TDE is currently
`DEFERRED_NOT_REPRODUCED` because no verified official checkpoint/protocol parity
was established.
...
Current gate audit is documented in `11_evidence_gate_audit.md`. The official
input check is blocked because `VG-SGG-with-attri.h5` and official TDE
checkpoints are not present in the workspace.
...
checkpoint, missing SGB-format VG roidb input, random-init fallback, tiny CPU
slice, and no official evaluator parity.
...
evaluation. Official TDE logic has been mapped to OpenSGG source locations, but
official checkpoint/runtime/evaluator parity is not established.
...
Provide official TDE checkpoints and SGB-format VG inputs, run official SGB
evaluation, then attempt strict OpenSGG checkpoint mapping only after causal
keys and untreated buffers are verified.
