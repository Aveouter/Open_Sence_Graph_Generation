    ...
    with open(path) as f:
        for line in f:
            ...
            feats[(d["img_id"], d["sub_idx"], d["obj_idx"])] = {
                "s": np.array(d["subj_feat"]),
                "o": np.array(d["obj_feat"]),
                "u": np.array(d["union_feat"]),
                "e": d["exposed_label"],
            }
    return feats
...
def filter_C(hm, ek):
    ...
    tp = (
        _PROJECT_ROOT
        / "outputs/gen_sgg/manifests"
        / "exp009_train_hidden_head_or_coarse_one_seed42.json"
    )
    with open(tp) as f:
        tm = json.load(f)
...
def main():
    ...
    with open(args.val_hidden_manifest) as f:
        hm = json.load(f)
    ep = (
        _PROJECT_ROOT
        / "outputs/gen_sgg/exp010_prior_strata/seed42/scores_exposed_conditional.jsonl"
    )
    ek = {}
    with open(ep) as f:
        for line in f:
            d = json.loads(line)
            ek[(d["image_id"], d["sub_idx"], d["obj_idx"])] = d
    kC = filter_C(hm, ek)
    kE = filter_E(hm, ek)
    ...
    for sn in ["C", "E"]:
        ...
        print(
            f"  mean cos: hidden={mc:.4f} exposed={me:.4f} gap={me - mc:.4f}",
            flush=True,
        )
    ...
    with open(out / "groundability.json", "w") as f:
